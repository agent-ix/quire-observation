...
2. Replace one fact at a time with the same semantic role and subject, derive the
   next revision, and retain the prior bytes for comparison.
3. Exercise missing predecessor, stale predecessor, skipped/non-increasing
   revision, branch, self-link, cross-authority, cross-scope, and cross-role
...
lineage, identity contradiction, component mutation, or one-over input refuses
before a bundle or validated view exists.
